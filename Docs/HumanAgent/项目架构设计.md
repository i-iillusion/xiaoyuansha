...
更新：2026-09-13。原架构记录依据 [GameManager](../../Scripts/GameManager.gd)、[EffectChain](../../Scripts/EffectChain.gd)、[DamageRecord](../../Scripts/DamageRecord.gd)、[RuleScheduler](../../Scripts/RuleScheduler.gd)、[TurnManager](../../Scripts/TurnManager.gd) 与 [Player](../../Scripts/Player.gd) 的静态阅读；现补充 PR #7 已合入的 [IdentityVictory](../../Scripts/IdentityVictory.gd)、[HandPayment](../../Scripts/HandPayment.gd)，以及本地候选 [FreeForAllVictory](../../Scripts/FreeForAllVictory.gd)、普通救援后的终局重查和青龙回合记录。CI 扩展尚待推送验证。
...
flowchart TD
    ...
    GM --> Victory[IdentityVictory / FreeForAllVictory：按模式只读判胜]
...
| 模块 | 当前承担什么 | 已知边界 |
| --- | --- | --- |
| MainMenu / Scenes | 模式入口、选将、场景及基础布局 | 本地候选开放 2/3 人乱斗与五人标准测试入口；当前四个对手锚点不足以承载 6～10 人布局 |
...
| HandPayment / 主动支付适配 | HandPayment 从普通物理手牌按声明或响应类型原子移除；GameManager 本地 ST-09 候选为 `determined_cards` 保留待支付原对象并复用主动出牌流程 | 不处理技能弃牌费用；`determined_cards` 的完整手牌计数/弃牌语义和装备区实例化仍未统一 |
...
| IdentityVictory | 按五人标准身份配比、生命状态和剩余身份给出胜方；无 UI、奖惩和状态写入 | 有濒死者则等待；非法配置、全员同时死亡、乱斗和奸雄不套用本模块；本地专项 51 条通过 |
| FreeForAllVictory | 按 2～10 人无身份名单和死亡状态给出唯一生还者或全灭平局；无 UI、奖励和状态写入 | 有濒死者则等待；拒绝有身份/重复/缺失角色；特殊可撤销死亡与召唤物仍需上层隔离；本地专项 36 条通过 |
