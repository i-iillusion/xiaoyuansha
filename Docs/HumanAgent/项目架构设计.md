...
普通伤害扣血后，`damage_applied` 先更新 UI；目标濒死时等待 `_resolve_dying`。该入口登记 DyingContext，依次执行 `_check_dying`（现有自救/装傻/贤者）→ 仍濒死才进 `before_death` → 仍濒死才 `_handle_death` → 释放窗口 → 重查终局；三个失血入口共用此流程。最终死亡处理依次确认死亡并登记防重入、公开非空身份、清理牌区、执行击杀奖惩、检查胜负；活跃窗口非提交阶段不能直接进入死亡。**完整预大习尚未接入：成功保留三个牌区，失败才进入最终死亡处理。** QA-T07/T08 的入口回归不等于完整技能验收。
